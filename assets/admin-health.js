(() => {
  ...
  async function render() {
    ...
    output.innerHTML = '<div class="health-detail-card health-detail-card--full"><p class="health-empty">Chargement des controles de sante...</p></div>';
    const health = await getJson('/api/health');
    if (!health) {
      updateSummary(0, 'critical', 'Indisponible', 'Aucune reponse de l API de sante.');
      output.innerHTML = '<div class="health-detail-card health-detail-card--full"><p class="health-empty">Impossible de joindre l API de sante. Reessaie dans quelques instants.</p></div>';
    ...
    const stateLabel = overallState === 'ok' ? 'Operationnel' : overallState === 'warning' ? 'Degrade' : 'Critique';
    const recommendationCount = Array.isArray(health.recommendations) ? health.recommendations.length : 0;
    updateSummary(score, overallState, stateLabel, healthy + ' / ' + providers.length + ' provider(s) operationnel(s)');
    setText('[data-health-provider-count]', providers.length);
    setText('[data-health-provider-meta]', healthy + ' operationnel(s), ' + (providers.length - healthy) + ' a surveiller');
    ...
    setText('[data-health-updated]', 'Derniere verification : ' + new Date().toLocaleTimeString('fr-FR'));
    const alert = document.querySelector('[data-health-alerts]');
    if (alert) {
      alert.dataset.state = overallState;
      alert.querySelector('strong').textContent = recommendationCount ? recommendationCount + ' recommandation(s) a examiner' : stateLabel;
      alert.querySelector('div span').textContent = recommendationCount ? 'La console detaillee contient les points d attention prioritaires.' : 'Les controles de disponibilite sont a jour.';
    }
    const providerRows = providers.length ? providers.map(([name, item]) => {
      const state = stateFor(item?.status);
      return '<div class="health-provider-row"><div><span class="health-provider-name">' + escapeHtml(name) + '</span><span class="health-provider-meta">OK ' + (item?.ok || 0) + ' / echec ' + (item?.fail || 0) + '</span></div><span class="health-state" data-state="' + state + '">' + labelFor(state) + '</span></div>';
    }).join('') : '<p class="health-empty">Aucun provider teste depuis le dernier redemarrage.</p>';
    ...
    const recommendations = recommendationCount ? health.recommendations.map((item) => '<p class="health-recommendation">' + escapeHtml(item) + '</p>').join('') : '<p class="health-empty">Rien a signaler pour le moment.</p>';
    output.innerHTML = '<article class="health-detail-card health-detail-card--wide"><div class="health-detail-card-header"><h3>Providers de donnees</h3><span class="health-state" data-state="' + overallState + '">' + healthy + '/' + providers.length + ' OK</span></div>' + providerRows + '</article>'
      + '<article class="health-detail-card"><h3>Marche actif</h3><p class="health-empty">' + escapeHtml(health.market?.forex?.label || 'Marche inconnu') + '</p><div class="health-learning-row"><span>Heure New York</span><strong>' + escapeHtml(health.market?.newYorkTime || '--') + '</strong></div></article>'
      ...
      + '<article class="health-detail-card"><h3>Apprentissage</h3><div class="health-learning-row"><span>Analyses</span><strong>' + (health.learning?.totalAnalyses || 0) + '</strong></div><div class="health-learning-row"><span>Winrate global</span><strong>' + escapeHtml(health.learning?.globalWinRate ?? 'non mesure') + '</strong></div></article>'
